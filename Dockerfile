...
RUN apt-get install -y --no-install-recommends curl wget unzip git

# Install CAPA rules via Github API
RUN curl -s https://api.github.com/repos/mandiant/capa-rules/releases/latest | grep -oP '"zipball_url":\s*"\K[^"]+' | xargs wget -q -O mandiant-rules.zip && \
    unzip mandiant-rules.zip -d rules 

RUN git clone --no-checkout https://github.com/mandiant/capa.git /capa && \
    cd /capa && \
